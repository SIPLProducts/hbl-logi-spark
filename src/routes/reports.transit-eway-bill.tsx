...
// Static option lists (mirrors Angular component's hard-coded dropdown values)
...
const TABLE_COLUMNS: { header: string; key: string; fallback?: boolean }[] = [
  { header: "Reference No", key: "REFERENCE_NUMBER" },
  { header: "In/Out", key: "INWARD_OUTWARD" },
  { header: "SAP Type", key: "SAP_NONSAP", fallback: true },
  { header: "Plant", key: "PLANT" },
  { header: "Division", key: "DIVISION" },
  { header: "Customer", key: "CUSTOMER" },
  { header: "Product", key: "PRODUCT", fallback: true },
  { header: "Material Desc", key: "MATERIAL_DESCRIPTION", fallback: true },
  ...
  { header: "Driver", key: "DRIVER_NAME", fallback: true },
  { header: "Mobile", key: "DRIVER_MOBILE", fallback: true },
  { header: "Dispatch Date", key: "PHYSICAL_DISPATCH_DATE" },
  { header: "ETA", key: "ETA", fallback: true },
  { header: "Eway No", key: "EWAY_NO", fallback: true },
  { header: "Eway Valid Till", key: "EWAY_VALIDITY_TILL", fallback: true },
...
// Mirrors Angular TransitReportsComponent.createArray
...
// Mirrors Angular TransitReportsComponent.formatDate
...
function TransitEwayBillReport() {
  ...
  const onSearch = async () => {
    ...
    try {
      const res: any = await service.FetchTransitReport(payload);
  ...
  const downloadPDF = () => {
    ...
    const tableRows = filteredData.map((row: any) => [
      ...
      row.DRIVER_NAME || "",
      row.DRIVER_MOBILE || "",
      formatDate(row.PHYSICAL_DISPATCH_DATE),
      row.ETA || "",
      row.EWAY_NO || "",
      ...
      <div className="bg-surface border border-hairline rounded-2xl shadow-elegant p-5 flex items-start gap-4">
        ...
        <div className="min-w-0">
          <h1 className="font-display text-[18px] font-bold tracking-tight text-indigo-700 dark:text-indigo-300">
            Transit & E-way bill Reports
          </h1>
          <p className="text-[12.5px] text-muted-foreground mt-1">
            Transit movements with linked E-way bill numbers and validity.
          </p>
        </div>
      </div>
                  ...
                  <tr key={i} className={i % 2 === 0 ? "bg-surface hover:bg-muted/50" : "bg-surface-2/40 hover:bg-muted/50"}>
                    {TABLE_COLUMNS.map((c) => (
                      <td key={c.key} className="px-3 py-2 whitespace-nowrap">
                        {c.fallback ? row[c.key] || "-" : row[c.key]}
                      </td>
                    ))}
                  </tr>
                ))}
