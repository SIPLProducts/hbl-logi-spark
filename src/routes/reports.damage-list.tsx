...
// It should expose the same functions you shared:
//   FetchDamageList, fetchVendorCode, getpdb, getssc, Incoterms
...
const TABLE_COLUMNS: { key: string; label: string }[] = [
  ...
  { key: "INCIDENT_CLOSING_DATE", label: "Incident Closing Date" },
];
...
const EMPTY_FORM: DamageListForm = {
  INOUT: [],
  SAPTYPE: [],
  FROM_DATE: "",
  TO_DATE: "",
...
export const Route = createFileRoute("/reports/damage-list")({
  component: DamageListReport,
});

function DamageListReport() {
  const [form, setForm] = useState<DamageListForm>(EMPTY_FORM);
...
const [customerList, setCustomerList] = useState<{ value: string; label: string }[]>([]);
  ...
  function setField<K extends keyof DamageListForm>(
    key: K,
    value: DamageListForm[K]
  ) {
  ...
  async function onSearch() {
    ...
    setLoading(true);
    try {
      const res: any = await service.FetchDamageList(payload);
        ...
        <div className="bg-surface border border-hairline rounded-2xl shadow-elegant p-5 flex items-start gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[12.5px] text-muted-foreground mt-1">
              Consolidated list of in-transit damages and resolutions.
            </p>
          </div>
        </div>
      </div>
...
/* ------------------------------------------------------------------ */
/*  Lightweight toast, replaces SweetAlert2                            */
/* ------------------------------------------------------------------ */